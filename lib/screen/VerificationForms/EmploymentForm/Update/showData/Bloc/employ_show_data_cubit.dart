...
import '../Model/employ_show_data_model.dart';
import 'employ_show_data_state.dart';


class EmployShowDataCubit extends Cubit<EmployShowDataState> {
  ApiService _apiService;

  EmployShowDataCubit(this._apiService)
      : super(EmployShowDataInitialState());
    ...
    try {
      ...
      if (response.data != null && response.data.containsKey("status")) {
        if (response.data["status"] == 200) {
          EmploymentShowDataModel employmentShowDataModel = EmploymentShowDataModel.fromJson(response.data);
